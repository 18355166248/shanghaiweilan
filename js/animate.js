...
//缓动框架;(普通属性和层级透明度)
function animate111(ele,json,fn){
    //要用定时器,先清定时器;
...
function animate11(ele,target) {
    clearInterval(ele.timer);//清除定时器 再凯奇新的定时器
    ele.timer = setInterval(function() {
        var st = target>ele.offsetLeft?13:-13;
        ele.style.left = ele.offsetLeft + st + "px";
        if (Math.abs(ele.offsetLeft-target)<=Math.abs(st)) {
